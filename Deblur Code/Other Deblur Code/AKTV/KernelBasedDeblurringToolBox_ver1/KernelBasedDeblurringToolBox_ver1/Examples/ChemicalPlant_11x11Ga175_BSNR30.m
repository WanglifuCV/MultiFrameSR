...
% read a test image
img = double(imread('ChemicalPlant256.bmp')); % Cameraman image

% create a PSF
PSFsupport = 11;
A = fspecial('ga', PSFsupport, 1.75); 

% blur the test image
imgb = conv2verge(img, A);

% add white Gaussian noise
BSNR = 30; % blurred signal to noise ration in [dB]
...
y_init = ishift2(real(ifft2(F)), (PSFsupport-1)/2, (PSFsupport-1)/2);
...
% parameter
h = 0.4; % the global smoothing parameter for the likelihood term
hu = 1.2; % the global smoothing parameter for the regularization term
nu = 0.3; % the step size
IT = 3400; % the number of iterations
ksize = 7; % the kernel support size
ktype = 'ga'; % the kernel support size
reg_lambda = 0.5; % the regularization parameter
radius = (ksize - 1) / 2;
...
for it = 1 : IT
    ...
    for i = -radius : 1 : radius
        for j = -radius : 1 : radius
            ...
            tmp = sign(y - conv2(ishift2(Utmp, -i, -j), A, 'same')) .* wt; % (Y - SSGU)
...
%     step = nu / ec_U_max;
    ...
    % display the image
    subplot(2,1,1); imagesc(ec_U(1+mg:N-mg, 1+mg:M-mg)); colormap(gray); axis image; colorbar;
    subplot(2,1,2); imshow(uint8(U(1+mg:N-mg, 1+mg:M-mg))); colormap(gray); axis image;
    title(['IT = ', num2str(it), ', RMSE = ', num2str(rmse)]);
    pause(0.1);
end

save ChemicalPlant_11x11Ga175_BSN30_L1L1_N1N1_RMSE85687 U Ux Uy;
