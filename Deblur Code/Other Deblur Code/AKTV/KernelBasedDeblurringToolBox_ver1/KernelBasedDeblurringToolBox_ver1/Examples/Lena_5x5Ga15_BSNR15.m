%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% STEERING KERNEL DEBLURRING EXAMPLE 3 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% the deblurred image of Fig.12(f) in the paper "Deblurring Using
% Locally-Adaptive Kernel regression.
%
% [Details]
% test image : cameraman
% PSF : 5 x 5 Gaussian with standard deviation 1.5
% BSNR : 15[dB]
%
% [History]
...
% read a test image
img = double(imread('lena.png')); % Lena image

% create a PSF
PSFsupport = 5;
A = fspecial('ga', PSFsupport, 1.5); % 5 x 5 Gaussian

% blur the test image
imgb = conv2verge(img, A);

% add white Gaussian noise
BSNR = 15; % blurred signal to noise ration in [dB]
...
fig_h = figure;
imshow(uint8(y(1+mg:end-mg, 1+mg:end-mg))); colormap(gray); axis image;
pause(0.1);
...
[U Ux Uy Uxx Uxy Uyy] = ckr_a(y, [0;0], 1, 1, Q, hc, ktype, ksize);
U = y;

% parameter
h = 1.75; % the global smoothing parameter for the likelihood term
hu = 2.5; % the global smoothing parameter for the regularization term
nu = 0.3; % the step size
IT = 500; % the number of iterations
ksize = 9; % the kernel support size
reg_lambda = 1.0; % the regularization parameter
radius = (ksize - 1) / 2;
...
save Lena_5x5Ga15_BSNR15_L2L1_N1N1_RMSE61216 U Ux Uy;